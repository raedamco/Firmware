...
//  Raedam 
//
//  Created on 10/16/2020. Modified on 10/16/2020 by Austin Mckee.
//  Copyright © 2020 Raedam. All rights reserved.
//
// This file holds code for the TCP communication between the sensors -> server -> database
...
// all communication functions inside here 
server.on('connection', function(socket){
  ...
  console.log('Socket timed out');
    ...
    // where data is recieved 
    socket.on('data',function(data){
    var the_data = Array.from(data);
    var Time = new Date();
    log("---------------------------------------------------------------------------------------------------------------------------------------------");
    log("PACKET RECIEVED: LENGTH: [" + the_data.length + "] | ADDRESS: [" + socket.remoteAddress + "] | PORT: [" + socket.remotePort + "] | TIME: [" + Time + "]");
     var SensorID = parseInt(the_data[])
});
