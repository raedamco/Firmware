...
server.on('message', (msg, info) => {
  ...
  const SensorID = msg.readUIntLE(0, 1);

  let Distance = ((((msg.readUIntLE(1, 2) * 0.000001) * 343)/2) * 39.37);
  Distance = Distance.toFixed(3);
  Distance = parseFloat(Distance, 10);
  const OccupiedDistance = 48; // Object is within 4 feet (48in)
...
async function test_function(sensorID, state, occupant, time, distance) {
  db.collection('Demos').doc('Parking Structure 1').collection('Floor 1').doc(sensorID).collection('Data').get().then((snap) => { // CHECK IF THERE ARE DOCUMENTS IN COLLECTION BEFORE MERGING, OTHERWISE ADD FIRST DOCUMENT
    if (snap.size == 0) {
      ...
      const old_data = db.collection('Demos').doc('Parking Structure 1').collection('Floor 1').doc(sensorID).collection('Data').orderBy('Time.End', 'desc').limit(1).get().then(async(querySnapshot) => {
        querySnapshot.forEach(async(doc) => {
          if (doc.data()['Occupant'] == occupant && doc.data()['Occupied'] == state)// checks for change in status if not log added to current doc
          {
            ...
            const the_test = await doc.data()['Distances'];
            typeof the_test;
            // log("The Test Type : "+ typeof the_test)
            the_test.push(distance);
            const temp_history = await doc.data()['Time']['History'];
            typeof temp_history;
...
function databaseListner() {
  db.collection('Demos').doc('Parking Structure 1').collection('Floor 1').onSnapshot((snapshot) => {
      ...
      if (change.type === 'modified') {
        if (change.doc.data()['Occupancy']['Occupied'] == true) {
          ...
          if (occupiedSpots.includes(change.doc.id)) {
            const index = occupiedSpots.indexOf(change.doc.id);
            if (index > -1) {
